...
public class JooqConditionBuilder {
    private final static ValueConversionHelper converter = new ValueConversionHelper();
    private static final String VALUE_SEPARATOR = ",";
    private static final String FILTER_VALUE_ALL = "all";


    public static <T extends IQueryField> List<Condition> buildConditions(List<FilterDescriptor<T, ?>> filterDescriptors) {
        ...
        if (CollectionUtils.isNotEmpty(filterDescriptors)) {
            for (FilterDescriptor<T, ?> filter : filterDescriptors) {
                ...
                Condition condition = buildCondition(field.getJooqField(), filter.getOperator(), filter.getValue());
        ...
        switch (operator) {
            ...
            case START_WITH:
                ...
                if (StringUtils.isNotBlank(filterValue)) {
                    filterValue = FilterOperator.START_WITH == operator ? filterValue + "%" : "%" + filterValue + "%";
            ...
            case IS_BLANK:
                ...
                if (field.getType().isAssignableFrom(String.class)) {
                    ...
                    blankFieldCondition = blankFieldCondition
                            .or(blankValue ? trimmedFieldLength.eq(0) : trimmedFieldLength.gt(0));
                }
